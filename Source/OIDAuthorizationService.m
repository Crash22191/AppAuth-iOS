...
static NSString* sAuthEndpointAddr;
static NSData*   sAuthEndpointCertificate;
...
#pragma mark URL Session Delegate class

@interface OIDAuthorizationServiceDelegate : NSObject <NSURLSessionDelegate>

@property NSString* authEndpointAddress;
@property NSData*   authEndPointCertificate;

@end

@implementation OIDAuthorizationServiceDelegate

@synthesize authEndpointAddress, authEndPointCertificate;
  ...
  completionHandler:(void (^)(NSURLSessionAuthChallengeDisposition, NSURLCredential * _Nullable))completionHandler
{
    ...
    {
        // First test if the server name is allowed
        if ( [challenge.protectionSpace.host isEqualToString:sAuthEndpointAddr] )
        {
        ...
        {
            // Then whether we have a cert configured for the server
            BOOL isValid = NO;
            if ( authEndPointCertificate != nil )
            {
                NSMutableArray* pinnedCertificates = [[NSMutableArray alloc] init];
                [pinnedCertificates addObject:(__bridge_transfer id)SecCertificateCreateWithData(NULL, (__bridge CFDataRef)authEndPointCertificate)];
                
                if (SecTrustSetAnchorCertificates(challenge.protectionSpace.serverTrust,
                                                  (__bridge CFArrayRef) pinnedCertificates) != errSecSuccess)
                {
                    NSLog(@"OAUTH: The anchor certificates couldn't be set.");
                }
                else
                {
...
{
    sAuthEndpointAddr = [url copy];
}

+ (void) setAuthEndpointCertificate:(NSData*) certData
{
    sAuthEndpointCertificate = [NSData dataWithData:certData];
}
...
+ (void)discoverServiceConfigurationForDiscoveryURL:(NSURL *)discoveryURL
    ...
    OIDAuthorizationServiceDelegate* delegate = [[OIDAuthorizationServiceDelegate alloc] init];
    delegate.authEndpointAddress = sAuthEndpointAddr;
    delegate.authEndPointCertificate = sAuthEndpointCertificate;
